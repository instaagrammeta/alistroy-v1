...
func (r *SellerRepository) FindBySlug(ctx context.Context, slug string) (*models.Seller, error) {
	var s models.Seller
	if err := r.db.WithContext(ctx).Preload("User").First(&s, "slug = ?", slug).Error; err != nil {
		return nil, err
	}
	return &s, nil
}

func (r *SellerRepository) FindByUserID(ctx context.Context, userID uuid.UUID) (*models.Seller, error) {
	var s models.Seller
...
func (r *SellerRepository) ExistsBySlug(ctx context.Context, slug string) (bool, error) {
	var count int64
	err := r.db.WithContext(ctx).Model(&models.Seller{}).Where("slug = ?", slug).Count(&count).Error
	return count > 0, err
}
...
func (r *SellerRepository) List(ctx context.Context, p ListSellersParams) ([]models.Seller, int64, error) {
	...
	if s := strings.TrimSpace(p.Search); s != "" {
		...
		q = q.Where("LOWER(name) LIKE ? OR LOWER(slug) LIKE ? OR LOWER(city) LIKE ?", like, like, like)
	}
	var total int64
	...
	err := q.Order("is_featured DESC, created_at DESC").
...
// TopSellers returns the sellers with the most approved products.
func (r *SellerRepository) Top(ctx context.Context, limit int) ([]models.Seller, error) {
	...
	if err := r.db.WithContext(ctx).Where("id IN ? AND status = ?", ids, models.SellerStatusApproved).
		Find(&sellers).Error; err != nil {
		return nil, err
	}
	// preserve order
	byID := make(map[uuid.UUID]models.Seller, len(sellers))
