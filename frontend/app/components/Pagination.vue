...
<template>
  <nav v-if="totalPages > 1" class="flex items-center justify-center gap-1 mt-8">
    ...
    <template v-for="(p, i) in pages" :key="i">
      ...
      <button v-else class="px-3 py-1.5 text-sm rounded-lg border" :class="p === page ? 'bg-brand-500 text-white border-brand-500' : 'border-gray-200 hover:bg-gray-50'" @click="emit('change', p as number)">{{ p }}</button>
    </template>
    <button class="btn-outline btn-sm" :disabled="page >= totalPages" @click="emit('change', page + 1)">›</button>
  </nav>
</template>
