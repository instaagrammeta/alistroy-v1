...
const exportUrl = computed(() => `${useApiBase()}/admin/export/products?status=${status.value}&q=${q.value}`)
  ...
  <div>
    <div class="flex items-center justify-between mb-4 gap-2">
      ...
      <div class="flex gap-2">
        ...
        <NuxtLink to="/admin/products/new" class="btn-primary btn-sm">{{ t('admin.new_product') }}</NuxtLink>
      </div>
    </div>
    ...
    <div class="card overflow-hidden">
      <table class="w-full text-sm">
        <thead class="bg-gray-50 text-left text-xs uppercase text-gray-500"><tr><th class="p-3">{{ t('common.name') }}</th><th class="p-3 hidden md:table-cell">{{ t('catalog.seller') }}</th><th class="p-3 hidden md:table-cell">{{ t('common.price') }}</th><th class="p-3">{{ t('common.status') }}</th><th class="p-3"></th></tr></thead>
        <tbody>
